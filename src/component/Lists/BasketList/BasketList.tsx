...
export default function BasketList() {
  const productBasket = useAppSelector(
    (state) => state.basketReducer.productBasket,
  );
  const productCount = useAppSelector(
    (state) => state.basketReducer.productCount,
  );
  ...
  const totalPrice = productBasket.reduce(
    (res, item) => res + item.price * productCount[item._id],
    0,
  );
  ...
  return (
    <>
      ...
      <button className={`btn btnPrimary ${styles.button}`}>
        Оформить заказ
      </button>
    </>
  );
